...
public class CameraFollower : MonoBehaviour
{
	...
	void Update()
	{
		...
		Vector3 velocityOffset = new Vector3(rigidbody.velocity.x * 0.7f, 0, -Mathf.Abs(rigidbody.velocity.x * 0.5f) - 5);

		Vector3 realTargetOffset = targetOffset + velocityOffset;

		this.transform.position = Vector3.Lerp(this.transform.position, objectToFollow.transform.position + realTargetOffset, 0.2f);
